...
// const byte RS485SoilHumidityRegistry[] = {0x01, 0x03, 0x00, 0x12, 0x00, 0x01, 0x24, 0x0F};
...
class MPSoilTemperatureSensor {   
  ...
  public:
    MPSoilTemperatureSensor(
      int sensorDeviceId, 
      float sensorCode, 
      String sensorLabel="mPSoilTemperatureSensor", 
    ...
    virtual float fetchData(){
        ...
        for (uint8_t i = 0; i < sizeof(MPSoilTemperatureRegisters); i++ ) mod.write( MPSoilTemperatureRegisters[i] );
